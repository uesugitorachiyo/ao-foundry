...
const (
	...
	pulseAtlasSchedulerInputSchema          = "ao.foundry.pulse-atlas-scheduler-input.v0.1"
	pulseRefactorClosurePacketSchema        = "ao.foundry.pulse-refactor-closure-packet.v0.1"
	...
	fullyUnsupervisedAuthoritySchema        = "ao.foundry.fully-unsupervised-complex-authority-gates-rollup.v0.1"
	fullyUnsupervisedAuthorityCommandSchema = "ao.command.fully-unsupervised-complex-authority-gates-readback.v0.1"
	firstNonPlanningStopGateClearanceSchema = "ao.foundry.fully-unsupervised-first-non-planning-stop-gate-clearance.v0.1"
	firstNonPlanningFinalRollupSchema       = "ao.foundry.fully-unsupervised-first-non-planning-final-rollup.v0.1"
...
func printHelp(w io.Writer) {
	...
	fmt.Fprintln(w, "  foundry fully-unsupervised first-non-planning stop-gate evaluate --workgraph <path> --stop-gate-graph <path> --node-gate <path> --run-link <path> --rollback <path> --sentinel <path> --promoter <path> --command-readback <path> --out <path> [--workgraph-out <path>]")
	fmt.Fprintln(w, "  foundry fully-unsupervised first-non-planning final-closure evaluate --workgraph <path> --run-links-root <path> --stop-gates-root <path> --out <path> [--mission-completion-out <path>] [--promoter-out <path>] [--command-readback-out <path>]")
...
func readFullyUnsupervisedClosureObject(name, path string) (MutationClassGateEvidence, map[string]any, bool, error) {
	if strings.TrimSpace(path) == "" {
		return MutationClassGateEvidence{}, nil, false, nil
	}
	if _, err := os.Stat(path); err != nil {
		if errors.Is(err, os.ErrNotExist) {
			return MutationClassGateEvidence{}, nil, false, nil
		}
		return MutationClassGateEvidence{}, nil, false, err
	}
	source, object, err := readComplexNodeGateObject(name, path)
	if err != nil {
		return MutationClassGateEvidence{}, nil, false, err
	}
	return source, object, true, nil
}
...
func runPulse(args []string, stdout, stderr io.Writer) int {
	...
	case "atlas-scheduler-input":
		return runPulseAtlasSchedulerInput(args[1:], stdout, stderr)
	case "closure-packet":
		return runPulseClosurePacket(args[1:], stdout, stderr)
...
func runPulseAtlasSchedulerInput(args []string, stdout, stderr io.Writer) int {
	...
	if *jsonOut {
		...
		fmt.Fprintf(stdout, "pulse_atlas_scheduler_input=%s\n", classGateString(result, "status"))
		fmt.Fprintf(stdout, "allowed_next_action=%s\n", classGateString(result, "allowed_next_action"))
		fmt.Fprintf(stdout, "selected_node_id=%s\n", classGateString(result, "selected_node_id"))
	...
	if classGateString(result, "status") != "ready" {
		return 1
	}
	return 0
}

func buildPulseAtlasSchedulerInput(workgraphPath, foundryImportPath string) (map[string]any, error) {
	...
	result["source_digests"] = appendPulseSource(result["source_digests"], workgraphSource)
	...
	result["source_digests"] = appendPulseSource(result["source_digests"], importSource)
	if classGateString(foundryImport, "contract_version") != atlasImportSchema {
	...
	nodes := classGateObjectSlice(workgraph["nodes"])
	counts := map[string]int{"total": len(nodes), "ready": 0, "blocked": 0, "completed": 0, "failed": 0}
	var selectedNode map[string]any
	for _, node := range nodes {
		status := classGateString(node, "status")
		if _, ok := counts[status]; ok {
			counts[status]++
		}
		if selectedNode == nil && status == "ready" {
			selectedNode = node
		}
	}
	...
	matchingTasks := []map[string]any{}
	for _, task := range classGateObjectSlice(foundryImport["tasks"]) {
		if classGateString(task, "node_id") == selectedNodeID {
			matchingTasks = append(matchingTasks, task)
		}
	}
	result["selected_node_id"] = selectedNodeID
	result["selected_task_id"] = selectedTaskID
	result["safe_node_foundry_import"] = map[string]any{
		"status":                         "blocked",
		"selected_node_id":               selectedNodeID,
		"selected_task_id":               selectedTaskID,
		"imported_task_count":            len(matchingTasks),
		"rejects_unselected_ready_nodes": true,
	}
	...
	result["blocking_next_actions"] = []string{}
	result["safe_node_foundry_import"] = map[string]any{
		"status":                         "ready",
		"selected_node_id":               selectedNodeID,
		"selected_task_id":               selectedTaskID,
...
func basePulseAtlasSchedulerInput() map[string]any {
	return map[string]any{
		"schema_version":        pulseAtlasSchedulerInputSchema,
		"status":                "blocked",
		"allowed_next_action":   "stop_blocked",
		"first_failing_check":   "",
		"blocking_next_actions": []string{},
		"selected_node_id":      "",
		"selected_task_id":      "",
		"atlas_compile_only":    true,
		"safe_node_foundry_import": map[string]any{
			"status":                         "blocked",
			"selected_node_id":               "",
			"selected_task_id":               "",
		...
		"task_counts":          map[string]int{"total": 0, "ready": 0, "blocked": 0, "completed": 0, "failed": 0},
		"source_digests":       []any{},
		"schedules_work":       false,
		"executes_work":        false,
		"approves_work":        false,
...
func failPulseAtlasSchedulerInput(result map[string]any, checkName, reason string) (map[string]any, error) {
	result["status"] = "blocked"
	result["allowed_next_action"] = "stop_blocked"
	result["first_failing_check"] = checkName
	result["blocking_next_actions"] = []string{"Regenerate Atlas workgraph and Foundry import so exactly one Atlas-selected ready node is present."}
	return result, errors.New(reason)
}
...
func buildPulseClosurePacket(paths map[string]string) (map[string]any, error) {
	...
	for _, requiredSource := range required {
		...
		result["source_digests"] = appendPulseSource(result["source_digests"], source)
		...
		result[requiredSource.key+"_status"] = status
		if requiredSource.key == "atlas_scheduler_input" {
			result["atlas_scheduler_status"] = status
		}
	}
	if strings.TrimSpace(paths["command_readback"]) != "" {
		...
		result["source_digests"] = appendPulseSource(result["source_digests"], source)
		result["command_readback_status"] = status
	}
	...
	result["selected_node_id"] = classGateString(scheduler, "selected_node_id")
	result["selected_task_id"] = classGateString(scheduler, "selected_task_id")
	if classGateString(result, "selected_node_id") == "" || classGateString(result, "selected_task_id") == "" {
...
func basePulseClosurePacket() map[string]any {
	return map[string]any{
		"schema_version":                 pulseRefactorClosurePacketSchema,
		"status":                         "blocked",
		"allowed_next_action":            "stop_event_loop",
		"first_failing_check":            "",
		"blocking_next_actions":          []string{},
		"blueprint_authorization_status": "missing",
		"atlas_scheduler_input_status":   "missing",
		"atlas_scheduler_status":         "missing",
		"intake_preflight_status":        "missing",
		"start_gate_status":              "missing",
		"runner_decision_status":         "missing",
		"event_loop_policy_status":       "missing",
		"command_readback_status":        "not_provided",
		"selected_node_id":               "",
		"selected_task_id":               "",
		"closes_refactor_slices":         []string{"D", "E"},
		"source_digests":                 []any{},
		"schedules_work":                 false,
		"executes_work":                  false,
		"approves_work":                  false,
...
func failPulseClosurePacket(result map[string]any, checkName, reason string) (map[string]any, error) {
	result["status"] = "blocked"
	result["allowed_next_action"] = "stop_event_loop"
	result["first_failing_check"] = checkName
	result["blocking_next_actions"] = []string{"Stop the Pulse event loop until closure evidence is regenerated and ready."}
	return result, errors.New(reason)
}

func loadPulseJSONSource(name, path string) (map[string]any, map[string]any, error) {
	if strings.TrimSpace(path) == "" {
		return nil, nil, errors.New("missing source path")
	}
	document, err := readArbitraryJSON(path)
	if err != nil {
	...
	object, ok := document.(map[string]any)
	if !ok {
		return nil, nil, fmt.Errorf("%s must be a JSON object", name)
	}
	if err := validatePublicSafeJSONStrings(object); err != nil {
		return nil, nil, err
	}
...
func pulseArtifactSourceFromFile(name, path, schemaVersion, status string) (map[string]any, error) {
	sum, err := fileSHA256(path)
	if err != nil {
		return nil, err
	}
...
func appendPulseSource(current any, source map[string]any) []any {
	sources, _ := current.([]any)
