...
func TestComplexRefactorRehearsalScriptIncludesRepairRepackAndCommandReadback(t *testing.T) {
	...
	for _, want := range []string{
		...
		"safe-node-foundry-import",
		"safe_node_selection",
		"selected_import_path",
		"safe_node_imported_tasks",
		"repair_plan",
		"context_repack",
		"command_readback",
	} {
  ...
  "repositories": [
    {
      ...
      "latest_ops": {
        ...
        "conclusion": "success",
        "run_id": "31172789285",
        "url": "https://github.com/uesugitorachiyo/ao-forge/actions/runs/31172789285"
      }
    }
  ],
  "next_actions": []
...
`
	...
	for _, want := range []string{
		...
		"| ao-forge | ci.yml | 31017677133 | already_recorded |",
...
func TestReleaseCandidateActiveStackParityBlocksStaleEvidence(t *testing.T) {
	...
	stale := strings.ReplaceAll(string(data), "main CI run 30968880090", "main CI run 28016224096")
	...
	for _, want := range []string{
		"release candidate active-stack parity: ao2-control-plane missing active-stack evidence \"main CI run 30968880090\"",
...
func TestFullyUnsupervisedAuthorityGatesEvaluateConsumesAllNodesAndDeniesLiveAuthority(t *testing.T) {
	dir := t.TempDir()
	artifacts := writeFullyUnsupervisedAuthorityGateArtifacts(t, dir, nil)
	outPath := filepath.Join(dir, "authority-gates-rollup.json")
	var stdout, stderr bytes.Buffer
	code := Run([]string{
		"fully-unsupervised", "authority-gates", "evaluate",
		"--blueprint-import", artifacts["blueprint_import"],
		"--workgraph", artifacts["workgraph"],
		"--foundry-import", artifacts["foundry_import"],
	...
	if code != 0 {
		t.Fatalf("authority gates evaluate returned %d, want 0; stdout=%s stderr=%s", code, stdout.String(), stderr.String())
	}
	rollup := readObjectFixture(t, outPath)
	if rollup["schema_version"] != "ao.foundry.fully-unsupervised-complex-authority-gates-rollup.v0.1" ||
		rollup["status"] != "denied" ||
		rollup["target_class"] != "fully_unsupervised_complex_mutation" ||
		rollup["highest_proven_live_class"] != "complex_repo_mutation" ||
		rollup["next_denied_class"] != "fully_unsupervised_complex_mutation" ||
		...
		rollup["fully_unsupervised_complex_mutation"] != "denied" ||
		rollup["rsi"] != "denied" {
		t.Fatalf("authority gates rollup must deny live authority: %#v", rollup)
	}
	if fmt.Sprint(rollup["nodes_consumed"]) != "3" ||
		fmt.Sprint(rollup["ready_nodes"]) != "1" ||
		fmt.Sprint(rollup["blocked_nodes"]) != "2" {
		t.Fatalf("authority gates rollup must consume all fixture nodes: %#v", rollup)
	}
	readback, ok := rollup["command_readback"].(map[string]any)
	if !ok ||
		readback["schema_version"] != "ao.command.fully-unsupervised-complex-authority-gates-readback.v0.1" ||
		readback["status"] != "ready" ||
		readback["safe_to_execute"] != false ||
		readback["fully_unsupervised_complex_mutation"] != "denied" ||
		readback["rsi"] != "denied" {
		t.Fatalf("rollup must include Command authority-gates denial readback: %#v", rollup["command_readback"])
	}
	checks, ok := rollup["checks"].(map[string]any)
	if !ok ||
		checks["continuation_handoff_ready"] != true ||
		checks["all_node_artifacts_present"] != true ||
		checks["all_nodes_non_executable"] != true ||
		checks["non_abandonment_evidence"] != true {
		t.Fatalf("authority-gates checks should pass while keeping class denied: %#v", checks)
	}
}

func TestFullyUnsupervisedAuthorityGatesEvaluateBlocksUnsafeCandidate(t *testing.T) {
	dir := t.TempDir()
	artifacts := writeFullyUnsupervisedAuthorityGateArtifacts(t, dir, func(paths map[string]string) {
		candidatePath := filepath.Join(paths["candidate_root"], "foundry-autonomous-runner-candidate.json")
		candidate := readObjectFixture(t, candidatePath)
		candidate["fully_unsupervised_complex_mutation_live_proven"] = true
		writeJSONFixtureForTest(t, candidatePath, candidate)
	})
	outPath := filepath.Join(dir, "authority-gates-rollup.json")
	var stdout, stderr bytes.Buffer
	code := Run([]string{
	...
	if code != 0 {
		t.Fatalf("blocked authority gates evaluate should still emit rollup, got %d; stdout=%s stderr=%s", code, stdout.String(), stderr.String())
	}
	rollup := readObjectFixture(t, outPath)
	if rollup["status"] != "blocked" ||
		rollup["safe_to_execute"] != false ||
		!objectStringSliceContains(rollup, "blockers", "candidate foundry-autonomous-runner must not claim fully_unsupervised_complex_mutation live-proven") {
		t.Fatalf("unsafe candidate must block authority-gates rollup: %#v", rollup)
	}
}

func TestFullyUnsupervisedAuthorityGatesEvaluateBlocksMissingNodeEvidence(t *testing.T) {
	dir := t.TempDir()
	artifacts := writeFullyUnsupervisedAuthorityGateArtifacts(t, dir, func(paths map[string]string) {
		if err := os.Remove(filepath.Join(paths["node_evidence_root"], "bounded-rehearsal-definition-evidence-requirements.json")); err != nil {
			t.Fatalf("remove node evidence fixture: %v", err)
		}
	})
	outPath := filepath.Join(dir, "authority-gates-rollup.json")
	var stdout, stderr bytes.Buffer
	...
	rollup := readObjectFixture(t, outPath)
	if rollup["status"] != "blocked" ||
		!objectStringSliceContains(rollup, "blockers", "node_evidence bounded-rehearsal-definition is missing or invalid") {
		t.Fatalf("missing node evidence must block authority-gates rollup: %#v", rollup)
	}
}
...
func TestRSIReadinessGateAllowsFirstBoundedEvidenceBaselineNode(t *testing.T) {
	dir := t.TempDir()
	artifacts := writeRSIFirstBoundedEvidenceArtifacts(t, dir, nil)
	outPath := filepath.Join(dir, "rsi-evidence-gate.json")
	var stdout, stderr bytes.Buffer
	code := Run([]string{
		"rsi", "readiness-gate", "evaluate",
		"--blueprint-import", artifacts["blueprint_import"],
	...
	if code != 0 {
		t.Fatalf("rsi evidence gate returned %d, want 0; stdout=%s stderr=%s", code, stdout.String(), stderr.String())
	}
	gate := readObjectFixture(t, outPath)
	if gate["schema_version"] != "ao.foundry.complex-repo-mutation-node-gate.v0.1" ||
		gate["status"] != "ready" ||
		gate["node_id"] != "baseline-metric-selection" ||
		gate["target_factory_repo"] != "ao-foundry" ||
		gate["safe_to_execute"] != true ||
		gate["highest_proven_live_class"] != "fully_unsupervised_complex_mutation" ||
		gate["next_denied_class"] != "RSI" ||
		gate["rsi"] != "denied" ||
		gate["authority_boundary"] != "rsi_evidence_baseline_gathering_only" {
		t.Fatalf("bounded evidence gate must unlock only baseline evidence: %#v", gate)
	}
}
...
func TestRSIReadinessGateBlocksBoundedEvidenceForbiddenSurfaceDrift(t *testing.T) {
	dir := t.TempDir()
	artifacts := writeRSIFirstBoundedEvidenceArtifacts(t, dir, func(paths map[string]string) {
		candidate := readObjectFixture(t, paths["candidate"])
		candidate["denied_surfaces"] = []any{
	...
	if code != 0 {
		t.Fatalf("blocked bounded evidence gate should still emit output, got %d; stdout=%s stderr=%s", code, stdout.String(), stderr.String())
	}
	gate := readObjectFixture(t, outPath)
	if gate["status"] != "blocked" ||
		gate["safe_to_execute"] != false ||
		!objectStringSliceContains(gate, "blockers", "RSI readiness candidate denied surfaces missing no prompt injection surface expansion") {
		t.Fatalf("forbidden surface drift must block bounded evidence gate: %#v", gate)
	}
}
...
func TestRSIReadinessStopGateAllowsNextNodeAfterClosureEvidence(t *testing.T) {
	dir := t.TempDir()
	artifacts := writeRSIReadinessStopGateArtifacts(t, dir, nil)
	outPath := filepath.Join(dir, "rsi-stop-gate-clearance.json")
	workgraphOut := filepath.Join(dir, "workgraph-after-stop-gate.json")
	var stdout, stderr bytes.Buffer
	code := Run([]string{
		"rsi", "readiness-stop-gate", "evaluate",
		"--workgraph", artifacts["workgraph_after_complete"],
		"--stop-gate-graph", artifacts["stop_gate_graph"],
		"--node-gate", artifacts["node_gate"],
		"--run-link", artifacts["run_link"],
		"--rollback", artifacts["rollback"],
		"--sentinel", artifacts["sentinel"],
		"--promoter", artifacts["promoter"],
		"--command-readback", artifacts["command_readback"],
		"--out", outPath,
		"--workgraph-out", workgraphOut,
	}, &stdout, &stderr)
	if code != 0 {
		t.Fatalf("RSI stop-gate evaluate returned %d, want 0; stdout=%s stderr=%s", code, stdout.String(), stderr.String())
	}
	clearance := readObjectFixture(t, outPath)
	if clearance["schema_version"] != rsiReadinessStopGateClearanceSchema ||
		clearance["status"] != "ready" ||
		clearance["safe_to_continue"] != true ||
		clearance["after_node"] != "rsi-definition" ||
		clearance["before_node"] != "carry-forward-fully-unsupervised-proof" ||
		clearance["fully_unsupervised_complex_mutation"] != "live-proven" ||
		clearance["rsi"] != "denied" {
		t.Fatalf("RSI stop-gate clearance must unlock only the next serialized node: %#v", clearance)
	}
	workgraph := readObjectFixture(t, workgraphOut)
	nodes := workgraph["nodes"].([]any)
	next := nodes[1].(map[string]any)
	if next["id"] != "carry-forward-fully-unsupervised-proof" || next["status"] != "ready" {
		t.Fatalf("next RSI readiness node must be ready after stop-gate clearance: %#v", next)
	}
}

func TestRSIReadinessStopGateBlocksMissingSentinel(t *testing.T) {
	dir := t.TempDir()
	artifacts := writeRSIReadinessStopGateArtifacts(t, dir, nil)
	outPath := filepath.Join(dir, "rsi-stop-gate-clearance.json")
	var stdout, stderr bytes.Buffer
	...
	if code != 0 {
		t.Fatalf("missing Sentinel should emit blocked RSI clearance, got %d; stdout=%s stderr=%s", code, stdout.String(), stderr.String())
	}
	clearance := readObjectFixture(t, outPath)
	if clearance["status"] != "blocked" ||
		clearance["safe_to_continue"] != false ||
		!objectStringSliceContains(clearance, "blockers", "Sentinel clear evidence is required") {
		t.Fatalf("missing Sentinel must block RSI stop gate: %#v", clearance)
	}
}

func TestRSIReadinessStopGateBlocksPromotionClaim(t *testing.T) {
	dir := t.TempDir()
	artifacts := writeRSIReadinessStopGateArtifacts(t, dir, func(paths map[string]string) {
		promoter := readObjectFixture(t, paths["promoter"])
		promoter["verdict"] = "promotion"
		writeJSONFixtureForTest(t, paths["promoter"], promoter)
	})
	outPath := filepath.Join(dir, "rsi-stop-gate-clearance.json")
	var stdout, stderr bytes.Buffer
	code := Run([]string{
	...
	if code != 0 {
		t.Fatalf("promotion claim should emit blocked RSI clearance, got %d; stdout=%s stderr=%s", code, stdout.String(), stderr.String())
	}
	clearance := readObjectFixture(t, outPath)
	if clearance["status"] != "blocked" ||
		clearance["safe_to_continue"] != false ||
		!objectStringSliceContains(clearance, "blockers", "Promoter evidence must be no_promotion") {
		t.Fatalf("promotion claim must block RSI stop gate: %#v", clearance)
	}
}

func TestRSIReadinessGateAllowsSerializedNodeAfterStopGateClearance(t *testing.T) {
	dir := t.TempDir()
	artifacts := writeRSIReadinessStopGateArtifacts(t, dir, nil)
	workgraph := readObjectFixture(t, artifacts["workgraph_after_complete"])
	nodes := workgraph["nodes"].([]any)
	next := nodes[1].(map[string]any)
	next["status"] = "ready"
	next["blockers"] = []any{}
	...
	workgraphPath := filepath.Join(dir, "workgraph-after-stop-gate.json")
	writeJSONFixtureForTest(t, workgraphPath, workgraph)
	nextScope := "factory/rsi-readiness-authority-boundary/01-carry-forward-fully-unsupervised-proof"
	requiredEvidence := []any{
		...
		"node_id:carry-forward-fully-unsupervised-proof",
		"node_class:proof carry-forward node",
	}
	nextTask := next["factory_task"].(map[string]any)
	nextTask["required_evidence"] = requiredEvidence
	nextTask["required_gates"] = []any{"atlas_blueprint_import", "rsi_definition", "command_readback", "public_claim_guard", "hidden_instruction_mutation_denied"}
	nextTask["rollback_scope"] = []any{nextScope}
	nextTask["safety_limits"] = []any{"no RSI execution from Atlas", "no unrestricted self-modification", "no policy auth secret provider deploy release expansion", "no credential use", "no dependency update", "no direct main mutation", "no hidden prompt or instruction mutation", "no public claim RSI proven", "no authority broadening"}
	nextImportPath := filepath.Join(dir, "foundry-import-after-stop-gate.json")
	writeJSONFixtureForTest(t, nextImportPath, map[string]any{
		"contract_version": "ao.atlas.foundry-import.v0.1",
		...
		"tasks": []any{
			map[string]any{
				"node_id":            "carry-forward-fully-unsupervised-proof",
				"task_id":            "carry-forward-fully-unsupervised-proof-task",
				"mutation_class":     "complex_repo_mutation",
				"write_scope":        []any{nextScope},
				"rollback_scope":     []any{nextScope},
				"required_evidence":  requiredEvidence,
				"required_gates":     nextTask["required_gates"],
				"authority_boundary": "blocked_until_predecessor_evidence_and_stop_gate_clear",
				"task":               nextTask,
			},
		},
	})
	candidatePath := filepath.Join(dir, "carry-forward-candidate.json")
	rollbackPath := filepath.Join(dir, "carry-forward-rollback.json")
	writeJSONFixtureForTest(t, candidatePath, map[string]any{
		...
		"highest_proven_live_class": "fully_unsupervised_complex_mutation",
		"status":                    "blocked",
		"candidate_class":           "proof carry-forward node",
		"selected_first_safe_node":  false,
		"safe_first_node_reason":    "blocked by serialized dependencies",
		"allowed_surfaces":          []any{nextScope},
		"denied_surfaces":           nextTask["safety_limits"],
		"required_evidence":         requiredEvidence,
	})
	writeJSONFixtureForTest(t, rollbackPath, map[string]any{
	...
	handoffPath := filepath.Join(dir, "handoff-after-stop-gate.json")
	writeJSONFixtureForTest(t, handoffPath, map[string]any{
		"contract_version":     "ao.atlas.foundry-continuation-handoff.v0.1",
		"first_safe_node":      "carry-forward-fully-unsupervised-proof",
		"total_node_count":     2,
	...
	stopGateClearancePath := filepath.Join(dir, "stop-gate-clearance.json")
	writeJSONFixtureForTest(t, stopGateClearancePath, map[string]any{
		"schema_version":                      rsiReadinessStopGateClearanceSchema,
		"status":                              "ready",
		"safe_to_continue":                    true,
		"workgraph_id":                        "rsi-readiness-authority-boundary-workgraph-test",
		"stop_gate_id":                        "stop-gate-01-rsi-definition-to-carry-forward-fully-unsupervised-proof",
		"after_node":                          "rsi-definition",
		"before_node":                         "carry-forward-fully-unsupervised-proof",
		"highest_proven_live_class":           "fully_unsupervised_complex_mutation",
		"next_denied_class":                   "RSI",
		"fully_unsupervised_complex_mutation": "live-proven",
		"rsi":                                 "denied",
	})
	outPath := filepath.Join(dir, "serialized-rsi-node-gate.json")
	var stdout, stderr bytes.Buffer
	code := Run([]string{
		"rsi", "readiness-gate", "evaluate",
		"--blueprint-import", artifacts["blueprint_import"],
		"--workgraph", workgraphPath,
		"--foundry-import", nextImportPath,
		"--continuation-handoff", handoffPath,
		"--atlas-summary", artifacts["atlas_summary"],
		"--slice-manifest", artifacts["slice_manifest"],
		"--final-synthesis", artifacts["final_synthesis"],
		"--candidate", candidatePath,
		"--rollback", rollbackPath,
		"--stop-gate-clearance", stopGateClearancePath,
		"--out", outPath,
	}, &stdout, &stderr)
	if code != 0 {
		t.Fatalf("serialized RSI readiness gate returned %d, want 0; stdout=%s stderr=%s", code, stdout.String(), stderr.String())
	}
	gate := readObjectFixture(t, outPath)
	if gate["status"] != "ready" ||
		gate["node_id"] != "carry-forward-fully-unsupervised-proof" ||
		gate["safe_to_execute"] != true ||
		gate["authority_boundary"] != "blocked_until_predecessor_evidence_and_stop_gate_clear" ||
		gate["rsi"] != "denied" {
		t.Fatalf("serialized RSI node should be ready only after stop-gate clearance: %#v", gate)
	}
}
...
func TestFullyUnsupervisedFirstNonPlanningGateAllowsSerializedNodeAfterStopGateClearance(t *testing.T) {
	dir := t.TempDir()
	artifacts := writeFullyUnsupervisedFirstNonPlanningStopGateArtifacts(t, dir, nil)
	workgraph := readObjectFixture(t, artifacts["workgraph_after_complete"])
	nodes := workgraph["nodes"].([]any)
	next := nodes[1].(map[string]any)
	next["status"] = "ready"
	...
	requiredEvidence := []any{
		...
		"node_id:no-reprompt-policy",
		"node_class:policy node",
	...
	writeJSONFixtureForTest(t, nextImportPath, map[string]any{
		...
		"tasks": []any{
			map[string]any{
				"node_id":            "no-reprompt-policy",
				"task_id":            "no-reprompt-policy-task",
				"mutation_class":     "complex_repo_mutation",
				"write_scope":        []any{"factory/fully-unsupervised-first-non-planning/01-no-reprompt-policy"},
				"rollback_scope":     []any{"factory/fully-unsupervised-first-non-planning/01-no-reprompt-policy"},
	...
	candidatePath := filepath.Join(dir, "no-reprompt-policy-candidate.json")
	rollbackPath := filepath.Join(dir, "no-reprompt-policy-rollback.json")
	writeJSONFixtureForTest(t, candidatePath, map[string]any{
		...
		"rsi":                                 "denied",
		"status":                              "blocked",
		"candidate_class":                     "policy node",
		"selected_first_safe_executable_node": false,
		"safe_first_node_reason":              "serialized behind predecessor stop gate",
		"allowed_surfaces":                    []any{"factory/fully-unsupervised-first-non-planning/01-no-reprompt-policy"},
		"denied_surfaces":                     []any{"no provider calls", "no credentials", "no dependency updates", "no auth policy config widening", "no secret env exposure", "no direct main mutation", "no concurrent mutation", "no RSI claim", "no release deploy publish upload tag"},
		"required_evidence":                   requiredEvidence,
	})
	writeJSONFixtureForTest(t, rollbackPath, map[string]any{
	...
	writeJSONFixtureForTest(t, handoffPath, map[string]any{
		...
		"command":              "codex --yolo",
		"first_safe_node":      "no-reprompt-policy",
		"total_node_count":     2,
		"completed_node_count": 1,
		"ready_node_count":     1,
		"blocked_node_count":   0,
		"class_boundary":       "Atlas import only for complex_repo_mutation; Foundry must preserve Atlas no-execution boundary",
		"schedules_work":       false,
		"executes_work":        false,
		"approves_work":        false,
	})
	...
	writeJSONFixtureForTest(t, stopGateClearancePath, map[string]any{
		...
		"safe_to_continue":                    true,
		"workgraph_id":                        "fully-unsupervised-complex-first-non-planning-workgraph-test",
		"stop_gate_id":                        "stop-gate-01-unsupervised-mission-ticket-to-no-reprompt-policy",
		"after_node":                          "unsupervised-mission-ticket",
		"before_node":                         "no-reprompt-policy",
		"highest_proven_live_class":           "complex_repo_mutation",
		"next_denied_class":                   "fully_unsupervised_complex_mutation",
		"fully_unsupervised_complex_mutation": "denied",
		"rsi":                                 "denied",
	})
	outPath := filepath.Join(dir, "serialized-node-gate.json")
	var stdout, stderr bytes.Buffer
	code := Run([]string{
		"fully-unsupervised", "first-non-planning", "gate", "evaluate",
		"--blueprint-import", artifacts["blueprint_import"],
	...
	if code != 0 {
		t.Fatalf("serialized first non-planning gate returned %d, want 0; stdout=%s stderr=%s", code, stdout.String(), stderr.String())
	}
	gate := readObjectFixture(t, outPath)
	if gate["status"] != "ready" ||
		gate["node_id"] != "no-reprompt-policy" ||
		gate["safe_to_execute"] != true ||
		gate["authority_boundary"] != "blocked_until_predecessor_terminal_evidence_and_stop_gate_clear" {
		t.Fatalf("serialized node should be ready only after stop-gate clearance: %#v", gate)
	}
}

func TestFullyUnsupervisedFirstNonPlanningStopGateAllowsNextNodeAfterClosureEvidence(t *testing.T) {
	dir := t.TempDir()
	artifacts := writeFullyUnsupervisedFirstNonPlanningStopGateArtifacts(t, dir, nil)
	outPath := filepath.Join(dir, "stop-gate-clearance.json")
	workgraphOut := filepath.Join(dir, "workgraph-after-stop-gate.json")
	var stdout, stderr bytes.Buffer
	code := Run([]string{
		"fully-unsupervised", "first-non-planning", "stop-gate", "evaluate",
		"--workgraph", artifacts["workgraph_after_complete"],
		"--stop-gate-graph", artifacts["stop_gate_graph"],
		"--node-gate", artifacts["node_gate"],
		"--run-link", artifacts["run_link"],
	...
	if code != 0 {
		t.Fatalf("stop-gate evaluate returned %d, want 0; stdout=%s stderr=%s", code, stdout.String(), stderr.String())
	}
	clearance := readObjectFixture(t, outPath)
	if clearance["schema_version"] != "ao.foundry.fully-unsupervised-first-non-planning-stop-gate-clearance.v0.1" ||
		clearance["status"] != "ready" ||
		clearance["safe_to_continue"] != true ||
		clearance["after_node"] != "unsupervised-mission-ticket" ||
		clearance["before_node"] != "no-reprompt-policy" ||
		clearance["fully_unsupervised_complex_mutation"] != "denied" ||
		clearance["rsi"] != "denied" {
		t.Fatalf("stop-gate clearance must unlock only the next serialized node: %#v", clearance)
	}
	workgraph := readObjectFixture(t, workgraphOut)
	nodes := workgraph["nodes"].([]any)
	next := nodes[1].(map[string]any)
	if next["id"] != "no-reprompt-policy" || next["status"] != "ready" {
		t.Fatalf("next node must be ready after stop-gate clearance: %#v", next)
	}
}

func TestFullyUnsupervisedFirstNonPlanningStopGateBlocksMissingSentinel(t *testing.T) {
	dir := t.TempDir()
	artifacts := writeFullyUnsupervisedFirstNonPlanningStopGateArtifacts(t, dir, nil)
	outPath := filepath.Join(dir, "stop-gate-clearance.json")
	var stdout, stderr bytes.Buffer
	...
	if code != 0 {
		t.Fatalf("missing sentinel should emit blocked clearance, got %d; stdout=%s stderr=%s", code, stdout.String(), stderr.String())
	}
	clearance := readObjectFixture(t, outPath)
	if clearance["status"] != "blocked" ||
		clearance["safe_to_continue"] != false ||
		!objectStringSliceContains(clearance, "blockers", "Sentinel clear evidence is required") {
		t.Fatalf("missing sentinel must block: %#v", clearance)
	}
}

func TestFullyUnsupervisedFirstNonPlanningStopGateBlocksPromotionClaim(t *testing.T) {
	dir := t.TempDir()
	artifacts := writeFullyUnsupervisedFirstNonPlanningStopGateArtifacts(t, dir, func(paths map[string]string) {
		promoter := readObjectFixture(t, paths["promoter"])
		promoter["verdict"] = "promotion"
		writeJSONFixtureForTest(t, paths["promoter"], promoter)
	})
	...
	if code != 0 {
		t.Fatalf("promotion claim should emit blocked clearance, got %d; stdout=%s stderr=%s", code, stdout.String(), stderr.String())
	}
	clearance := readObjectFixture(t, outPath)
	if clearance["status"] != "blocked" ||
		clearance["safe_to_continue"] != false ||
		!objectStringSliceContains(clearance, "blockers", "Promoter evidence must be no_promotion") {
		t.Fatalf("promotion claim must block: %#v", clearance)
	}
}
...
func writeRSIFirstBoundedEvidenceArtifacts(t *testing.T, dir string, mutate func(map[string]string)) map[string]string {
	t.Helper()
	scope := "factory/rsi-first-bounded-evidence/00-baseline-metric-selection"
	paths := map[string]string{
		"blueprint_import":     filepath.Join(dir, "blueprint-import.json"),
		"workgraph":            filepath.Join(dir, "workgraph.json"),
		"foundry_import":       filepath.Join(dir, "foundry-import.json"),
		...
		"slice_manifest":       filepath.Join(dir, "sdd-slice-completion-manifest.json"),
		"final_synthesis":      filepath.Join(dir, "final-evidence-synthesis.json"),
		"candidate":            filepath.Join(dir, "baseline-metric-selection-candidate.json"),
		"rollback":             filepath.Join(dir, "baseline-metric-selection-rollback.json"),
	}
	requiredEvidence := []any{
		"highest_proven_live_class:fully_unsupervised_complex_mutation",
		"next_denied_class:RSI",
		...
		"hidden_instruction_mutation_denied:true",
		"node_id:baseline-metric-selection",
		"node_class:baseline metric node",
	}
	requiredGates := []any{
		"atlas_blueprint_import",
		"carry_forward_fully_unsupervised_proof",
		"rsi_readiness_final_denial",
		"baseline_metric_selection",
		"baseline_measurement",
		"candidate_proposal",
		"risk_classification",
		"covenant_exact_scope_ticket",
		"foundry_gated_execution",
		"forge_ao2_bounded_packet",
		"sentinel_hold_clear",
		"promoter_verdict",
		"command_readback",
		"regression_eval_suite",
		"reversible_change_proof",
		"rollback_rehearsal",
		"post_change_measurement",
		"improvement_threshold",
		"public_claim_guard",
		"hidden_instruction_denial",
		"broad_rsi_denial",
	}
	safetyLimits := []any{
		"docs test readback evaluation improvement only",
	...
	task := map[string]any{
		"contract_version":    "ao.atlas.factory-task.v0.1",
		"id":                  "baseline-metric-selection-task",
		"objective":           "Baseline metric selection for the first bounded RSI evidence rehearsal.",
		"target_factory_repo": "ao-foundry",
		"factory_folder":      scope,
		"mutation_class":      "complex_repo_mutation",
		"acceptance_criteria": []any{"baseline metric node evidence is complete", "RSI remains denied until final synthesis proves a bounded transition", "no hidden instruction mutation or authority broadening occurs"},
		"non_goals":           []any{"do not execute RSI from Atlas", "do not claim RSI proven", "do not apply hidden self-modification"},
		"write_scope":         []any{scope},
		"required_gates":      requiredGates,
		"rollback_scope":      []any{scope},
		"required_evidence":   requiredEvidence,
		"safety_limits":       safetyLimits,
		"authority_boundary":  "rsi_evidence_baseline_gathering_only",
	}
	writeJSONFixtureForTest(t, paths["workgraph"], map[string]any{
		"contract_version": "ao.atlas.workgraph.v0.1",
		"id":               "rsi-first-bounded-evidence-rehearsal-workgraph-test",
		"nodes": []any{
			map[string]any{"id": "baseline-metric-selection", "status": "ready", "factory_task": task, "dependencies": []any{}, "blockers": []any{}},
			map[string]any{"id": "carry-forward-evidence", "status": "blocked", "factory_task": map[string]any{
				"id":                 "carry-forward-evidence-task",
				"mutation_class":     "complex_repo_mutation",
				"write_scope":        []any{"factory/rsi-first-bounded-evidence/01-carry-forward-evidence"},
				"authority_boundary": "blocked_until_predecessor_terminal_evidence_and_stop_gate_clear",
			}, "dependencies": []any{"baseline-metric-selection"}, "blockers": []any{"waits for predecessor terminal evidence"}},
		},
	})
	writeJSONFixtureForTest(t, paths["blueprint_import"], map[string]any{
		"contract_version":           "ao.atlas.blueprint-import.v0.1",
		"id":                         "rsi-first-bounded-evidence-blueprint-import-test",
		"status":                     "ready",
	...
	writeJSONFixtureForTest(t, paths["foundry_import"], map[string]any{
		"contract_version": "ao.atlas.foundry-import.v0.1",
		"id":               "rsi-first-bounded-evidence-foundry-import-test",
		"workgraph_id":     "rsi-first-bounded-evidence-rehearsal-workgraph-test",
		"target_instance":  "demo-stack",
		"status":           "ready_for_foundry_fixture_import",
		"schedules_work":   false,
		...
		"tasks": []any{
			map[string]any{
				"node_id":            "baseline-metric-selection",
				"task_id":            "baseline-metric-selection-task",
				"mutation_class":     "complex_repo_mutation",
				"write_scope":        []any{scope},
				"rollback_scope":     []any{scope},
				"required_gates":     requiredGates,
	...
	writeJSONFixtureForTest(t, paths["continuation_handoff"], map[string]any{
		"contract_version":     "ao.atlas.foundry-continuation-handoff.v0.1",
		"first_safe_node":      "baseline-metric-selection",
		"total_node_count":     2,
		"completed_node_count": 0,
		"ready_node_count":     1,
		"blocked_node_count":   1,
	...
	writeJSONFixtureForTest(t, paths["atlas_summary"], map[string]any{
		"schema":                       "ao.atlas.private-rsi-evidence-summary.v0.1",
		"first_safe_node":              "baseline-metric-selection",
		"planned_node_count":           2,
		"ready_node_count":             1,
		"blocked_node_count":           1,
		...
		"next_denied_class":            "RSI",
		"foundry_import_task_count":    1,
		"foundry_import_selected_node": "baseline-metric-selection",
	})
	writeJSONFixtureForTest(t, paths["slice_manifest"], map[string]any{
		"schema":           "ao.atlas.private-rsi-evidence-slice-manifest.v0.1",
		"total_slices":     8,
		"completed_slices": 8,
		"rsi":              "denied",
	})
	writeJSONFixtureForTest(t, paths["final_synthesis"], map[string]any{
		"schema":              "ao.atlas.private-rsi-evidence-final-evidence-synthesis.v0.1",
		"first_safe_node":     "baseline-metric-selection",
		"planned_node_count":  2,
		"ready_node_count":    1,
		"blocked_node_count":  1,
		...
		"rsi_live_proven":     false,
		"live_self_modification_performed_by_atlas":      false,
		"first_bounded_rsi_evidence_rehearsal_requested": true,
		"highest_proven_live_class":                      "fully_unsupervised_complex_mutation",
		"next_denied_class":                              "RSI",
		"foundry_import_task_count":                      1,
		"foundry_import_selected_node":                   "baseline-metric-selection",
	...
	writeJSONFixtureForTest(t, paths["candidate"], map[string]any{
		"schema":                    "ao.atlas.private-rsi-evidence-candidate.v0.1",
		"node_id":                   "baseline-metric-selection",
		"task_id":                   "baseline-metric-selection-task",
		"rsi":                       "denied",
		"highest_proven_live_class": "fully_unsupervised_complex_mutation",
		"status":                    "ready",
		"candidate_class":           "baseline metric node",
		"selected_first_safe_node":  true,
		"safe_first_node_reason":    "baseline metric selection and evidence gathering only",
		"allowed_surfaces":          []any{scope},
		"denied_surfaces":           safetyLimits,
		"required_evidence":         requiredEvidence,
	})
	writeJSONFixtureForTest(t, paths["rollback"], map[string]any{
		"schema":                    "ao.atlas.private-rsi-evidence-rollback.v0.1",
		"node_id":                   "baseline-metric-selection",
		"task_id":                   "baseline-metric-selection-task",
		"rsi":                       "denied",
		"highest_proven_live_class": "fully_unsupervised_complex_mutation",
		"rollback_scope":            []any{scope},
		"rollback_plan":             []any{"remove generated node artifacts", "restore RSI denied readback", "record rollback evidence", "stop on boundary drift"},
		"auto_stop_triggers":        []any{"Sentinel hold", "kill switch", "hidden instruction mutation", "authority broadening", "rollback failure", "public RSI claim"},
	})
	if mutate != nil {
		mutate(paths)
	}
...
func writeRSIReadinessStopGateArtifacts(t *testing.T, dir string, mutate func(map[string]string)) map[string]string {
	t.Helper()
	base := writeRSIReadinessArtifacts(t, dir, nil)
	paths := map[string]string{}
	for key, value := range base {
		paths[key] = value
	}
	paths["workgraph_after_complete"] = filepath.Join(dir, "workgraph-after-complete.json")
	paths["stop_gate_graph"] = filepath.Join(dir, "stop-gate-graph.json")
	paths["node_gate"] = filepath.Join(dir, "node-gate.json")
	paths["run_link"] = filepath.Join(dir, "run-link.json")
	paths["sentinel"] = filepath.Join(dir, "sentinel.json")
	paths["promoter"] = filepath.Join(dir, "promoter.json")
	paths["command_readback"] = filepath.Join(dir, "command-readback.json")
	workgraph := readObjectFixture(t, paths["workgraph"])
	nodes := workgraph["nodes"].([]any)
	first := nodes[0].(map[string]any)
	first["status"] = "completed"
	nodes[0] = first
	workgraph["nodes"] = nodes
	writeJSONFixtureForTest(t, paths["workgraph_after_complete"], workgraph)
	writeJSONFixtureForTest(t, paths["stop_gate_graph"], map[string]any{
		"schema":          "ao.atlas.private-rsi-readiness-stop-gate-graph.v0.1",
		"workgraph_id":    "rsi-readiness-authority-boundary-workgraph-test",
		"stop_gate_count": 1,
		"stop_gates": []any{
			map[string]any{
				"id":          "stop-gate-01-rsi-definition-to-carry-forward-fully-unsupervised-proof",
				"after_node":  "rsi-definition",
				"before_node": "carry-forward-fully-unsupervised-proof",
				"required_evidence": []any{
					...
					"RSI denied readback",
					"rollback record",
					"Sentinel hold or clear",
					"Promoter verdict",
					"Command readback",
					"public claim guard",
				},
				"release_condition": "next node may become ready only after all gate evidence is recorded",
				"hard_stop_conditions": []any{
	...
	writeJSONFixtureForTest(t, paths["node_gate"], map[string]any{
		"schema_version":                      "ao.foundry.complex-repo-mutation-node-gate.v0.1",
		"status":                              "ready",
		"mutation_class":                      "complex_repo_mutation",
		"highest_proven_live_class":           "fully_unsupervised_complex_mutation",
	...
	writeJSONFixtureForTest(t, paths["run_link"], map[string]any{
		"contract_version": atlasRunLinkSchema,
		"task_id":          "rsi-definition-task",
		"status":           "completed",
		"evidence": map[string]any{
			"changed_file": "factory/rsi-readiness-authority-boundary/00-rsi-definition/node-record.json",
			"ci":           "passed",
			"merge_commit": "abc123",
			"node_gate":    paths["node_gate"],
			"pr":           "https://github.com/uesugitorachiyo/ao-foundry/pull/164",
		},
	})
	writeJSONFixtureForTest(t, paths["sentinel"], map[string]any{
	...
	writeJSONFixtureForTest(t, paths["promoter"], map[string]any{
	...
	writeJSONFixtureForTest(t, paths["command_readback"], map[string]any{
...
func writeFullyUnsupervisedFirstNonPlanningStopGateArtifacts(t *testing.T, dir string, mutate func(map[string]string)) map[string]string {
	t.Helper()
	base := writeFullyUnsupervisedFirstNonPlanningArtifacts(t, dir, nil)
	paths := map[string]string{}
	for key, value := range base {
		paths[key] = value
	}
	...
	writeJSONFixtureForTest(t, paths["workgraph_after_complete"], workgraph)
	writeJSONFixtureForTest(t, paths["stop_gate_graph"], map[string]any{
		"schema":          "ao.atlas.private-first-non-planning-stop-gate-graph.v0.1",
		"workgraph_id":    "fully-unsupervised-complex-first-non-planning-workgraph-test",
		"stop_gate_count": 1,
		"stop_gates": []any{
			map[string]any{
				"id":          "stop-gate-01-unsupervised-mission-ticket-to-no-reprompt-policy",
				"after_node":  "unsupervised-mission-ticket",
				"before_node": "no-reprompt-policy",
				"required_evidence": []any{
					"predecessor terminal evidence",
					"no-reprompt proof",
					"rollback record",
					"Sentinel clear or hold record",
					"Promoter no-promotion",
					"Command readback",
					"public claim guard",
				},
				"release_condition": "next node may become ready only after all gate evidence is recorded",
	...
	writeJSONFixtureForTest(t, paths["run_link"], map[string]any{
		...
		"status":           "completed",
		"evidence": map[string]any{
			"changed_file":  "factory/fully-unsupervised-first-non-planning/00-unsupervised-mission-ticket/node-record.json",
			"ci":            "passed",
			"merge_commit":  "abc123",
			"node_gate":     paths["node_gate"],
			"pr":            "https://github.com/uesugitorachiyo/ao-foundry/pull/134",
			"public_claims": "guarded",
		},
	})
	writeJSONFixtureForTest(t, paths["sentinel"], map[string]any{
		"schema_version":                      "ao.sentinel.fully-unsupervised-first-non-planning-stop-gate-verdict.v0.1",
...
func writeFullyUnsupervisedAuthorityGateArtifacts(t *testing.T, dir string, mutate func(map[string]string)) map[string]string {
	t.Helper()
	paths := map[string]string{
		"blueprint_import":     filepath.Join(dir, "blueprint-import.json"),
		"workgraph":            filepath.Join(dir, "workgraph.json"),
	...
	nodes := []struct {
		...
		{"authority-gate-support-readback", "authority-gate-support-readback-task", "ready", "support/readback authority-gate node", "00-authority-gate-support-readback"},
		{"bounded-rehearsal-definition", "bounded-rehearsal-definition-task", "blocked", "bounded rehearsal definition", "01-bounded-rehearsal-definition"},
		{"foundry-autonomous-runner", "foundry-autonomous-runner-task", "blocked", "Foundry autonomous runner", "02-foundry-autonomous-runner"},
	}
	workgraphNodes := []any{}
	requiredGates := []any{
		"atlas_blueprint_import",
		"non_planning_authority_gate",
		"covenant_unsupervised_authority_ticket",
		"forge_ao2_bounded_packet",
		"per_node_authority_envelope",
		"foundry_continuation_handoff",
	...
	requiredEvidence := []any{
		...
		"next_denied_class:fully_unsupervised_complex_mutation",
		"rsi:denied",
		"foundry_pr_131_readiness_only_denial:bound",
		"no_return_early_manifest:required",
		"safe_to_execute:false",
	}
	for i, node := range nodes {
		dependencies := []any{}
		...
		if i > 0 {
			dependencies = append(dependencies, nodes[i-1].ID)
			blockers = append(blockers, "waits for predecessor terminal authority-gate evidence")
		}
		nodeRequiredEvidence := append([]any{}, requiredEvidence...)
		nodeRequiredEvidence = append(nodeRequiredEvidence, "node_id:"+node.ID, "node_class:"+node.NodeClass)
		task := map[string]any{
			"contract_version":    "ao.atlas.factory-task.v0.1",
			"id":                  node.TaskID,
			"objective":           "Authority gate evidence for " + node.NodeClass,
			"target_factory_repo": "ao-foundry",
			"factory_folder":      "factory/fully-unsupervised-authority-gates/" + node.Folder,
			"mutation_class":      "complex_repo_mutation",
			"acceptance_criteria": []any{
				node.NodeClass + " authority-gate evidence is complete",
				"fully_unsupervised_complex_mutation remains denied",
				"RSI remains denied",
			},
			"non_goals": []any{
				"do not execute ungated mutation",
				"do not claim fully_unsupervised_complex_mutation proven",
				"do not claim RSI proven",
			},
			"write_scope":       []any{"factory/fully-unsupervised-authority-gates/" + node.Folder},
			"required_gates":    requiredGates,
			"rollback_scope":    []any{"factory/fully-unsupervised-authority-gates/" + node.Folder},
			"required_evidence": nodeRequiredEvidence,
			"safety_limits": []any{
				"no ungated mutation",
				"no RSI",
				"no release deploy publish upload tag",
			...
			"authority_boundary": "bounded_authority_gate_support_readback_only",
		}
		writeJSONFixtureForTest(t, filepath.Join(paths["task_root"], node.ID+"-task.json"), task)
		writeJSONFixtureForTest(t, filepath.Join(paths["context_root"], node.ID+"-context.json"), map[string]any{
			"contract_version": "ao.atlas.context-pack.v0.1",
			"node_id":          node.ID,
			"task_id":          node.TaskID,
			"status":           "ready",
		})
		writeJSONFixtureForTest(t, filepath.Join(paths["candidate_root"], node.ID+"-candidate.json"), map[string]any{
			"schema":                   "ao.atlas.private-authority-gate-candidate.v0.1",
			"node_id":                  node.ID,
			"task_id":                  node.TaskID,
			"status":                   node.Status,
			"candidate_class":          node.NodeClass,
			"selected_first_safe_node": i == 0,
			"safe_first_node_reason":   "support/readback authority-gate work only",
			"allowed_surfaces":         []any{"factory/fully-unsupervised-authority-gates/" + node.Folder},
			"denied_surfaces":          task["safety_limits"],
			"required_evidence":        nodeRequiredEvidence,
			"fully_unsupervised_complex_mutation_live_proven": false,
			"rsi": "denied",
		})
		writeJSONFixtureForTest(t, filepath.Join(paths["rollback_root"], node.ID+"-rollback.json"), map[string]any{
			"schema":             "ao.atlas.private-authority-gate-rollback.v0.1",
			"node_id":            node.ID,
			"task_id":            node.TaskID,
			"rollback_scope":     []any{"factory/fully-unsupervised-authority-gates/" + node.Folder},
			"rollback_plan":      []any{"remove generated authority-gate artifacts for this node", "restore prior denial readback"},
			"auto_stop_triggers": []any{"CI failure", "Sentinel hold", "public claim broadening", "forbidden surface expansion", "RSI boundary crossing"},
			"fully_unsupervised_complex_mutation_live_proven": false,
			"rsi": "denied",
		})
		writeJSONFixtureForTest(t, filepath.Join(paths["node_evidence_root"], node.ID+"-evidence-requirements.json"), map[string]any{
			"schema":                     "ao.atlas.private-authority-gate-node-evidence-requirements.v0.1",
			"node_id":                    node.ID,
			"task_id":                    node.TaskID,
			"terminal_evidence_required": true,
			"accepted_terminal_states":   []any{"completed", "blocked", "repaired", "repacked", "rolled_back", "denied"},
			"required_evidence":          nodeRequiredEvidence,
			"non_abandonment_rule":       "node cannot disappear from mission manifest without terminal evidence",
			"fully_unsupervised_complex_mutation_live_proven": false,
			"rsi": "denied",
		})
		writeJSONFixtureForTest(t, filepath.Join(paths["repair_root"], node.ID+"-repair-plan.json"), map[string]any{
			"schema":          "ao.atlas.private-authority-gate-repair-plan.v0.1",
			"node_id":         node.ID,
			"task_id":         node.TaskID,
			"status":          "repair_planned",
			"safe_to_execute": false,
		...
		writeJSONFixtureForTest(t, filepath.Join(paths["repack_root"], node.ID+"-context-repack-plan.json"), map[string]any{
			"schema":          "ao.atlas.private-authority-gate-context-repack-plan.v0.1",
			"node_id":         node.ID,
			"task_id":         node.TaskID,
			"status":          "context_repack_planned",
			"safe_to_execute": false,
	...
	writeJSONFixtureForTest(t, paths["workgraph"], map[string]any{
		"contract_version": "ao.atlas.workgraph.v0.1",
		"id":               "fully-unsupervised-complex-authority-gates-workgraph-test",
		"nodes":            workgraphNodes,
	})
	writeJSONFixtureForTest(t, paths["blueprint_import"], map[string]any{
		"contract_version":           "ao.atlas.blueprint-import.v0.1",
		"id":                         "fully-unsupervised-complex-authority-gates-blueprint-import-test",
		"project_id":                 "fully_unsupervised_complex_mutation-authority-gates-blueprint",
		"status":                     "ready",
		"workgraph_id":               "fully-unsupervised-complex-authority-gates-workgraph-test",
		"mutation_class":             "complex_repo_mutation",
		"ready_for_foundry":          true,
		"safe_to_execute":            false,
		"live_execution_proven":      false,
		...
		"candidate_selection": map[string]any{
			...
			"approves_work":         false,
			"mutates_repositories":  false,
			"required_evidence":     append(append([]any{}, requiredEvidence...), "non_planning_authority_gates_requested:true"),
		},
	})
	writeJSONFixtureForTest(t, paths["foundry_import"], map[string]any{
		"contract_version": "ao.atlas.foundry-import.v0.1",
		"id":               "fully-unsupervised-complex-authority-gates-foundry-import-test",
		"workgraph_id":     "fully-unsupervised-complex-authority-gates-workgraph-test",
		"status":           "ready_for_foundry_fixture_import",
		"schedules_work":   false,
		"executes_work":    false,
		...
		"tasks": []any{
			map[string]any{
				...
				"task_id":            nodes[0].TaskID,
				"mutation_class":     "complex_repo_mutation",
				"write_scope":        []any{"factory/fully-unsupervised-authority-gates/00-authority-gate-support-readback"},
				"rollback_scope":     []any{"factory/fully-unsupervised-authority-gates/00-authority-gate-support-readback"},
				"required_gates":     requiredGates,
				"required_evidence":  append(append([]any{}, requiredEvidence...), "node_id:"+nodes[0].ID),
				"authority_boundary": "bounded_authority_gate_support_readback_only",
			},
		},
	})
	writeJSONFixtureForTest(t, paths["continuation_handoff"], map[string]any{
		"contract_version":     "ao.atlas.foundry-continuation-handoff.v0.1",
		"id":                   "fully-unsupervised-complex-authority-gates-continuation-handoff-test",
		"target_folder":        "/" + "Users/torachiyouesugi/Documents/public/ao-foundry",
		"command":              "codex --yolo",
		"first_safe_node":      nodes[0].ID,
		"total_node_count":     len(nodes),
		"completed_node_count": 0,
		"ready_node_count":     1,
		"blocked_node_count":   2,
		"class_boundary":       "complex_repo_mutation is live-proven; this handoff only requests bounded authority gates for fully_unsupervised_complex_mutation and preserves the Atlas no-execution boundary",
		"schedules_work":       false,
		"executes_work":        false,
		"approves_work":        false,
	})
	writeJSONFixtureForTest(t, paths["atlas_summary"], map[string]any{
		"schema":                    "ao.atlas.private-authority-gate-summary.v0.1",
		"first_safe_node":           nodes[0].ID,
		"planned_node_count":        len(nodes),
		"ready_node_count":          1,
		"blocked_node_count":        2,
		"context_pack_count":        len(nodes),
		"candidate_record_count":    len(nodes),
		"rollback_record_count":     len(nodes),
		"repair_plan_count":         len(nodes),
		"context_repack_plan_count": len(nodes),
		"sdd_slice_count":           30,
		"completed_sdd_slice_count": 30,
		"safe_to_execute":           false,
		"fully_unsupervised_complex_mutation_live_proven": false,
		"rsi":                           "denied",
		"highest_proven_live_class":     "complex_repo_mutation",
	...
	writeJSONFixtureForTest(t, paths["slice_manifest"], map[string]any{
		"schema":           "ao.atlas.private-authority-gate-slice-manifest.v0.1",
		"total_slices":     30,
		"completed_slices": 30,
		"rsi":              "denied",
	})
	writeJSONFixtureForTest(t, paths["final_synthesis"], map[string]any{
		"schema":                    "ao.atlas.private-authority-gate-final-evidence-synthesis.v0.1",
		"highest_proven_live_class": "complex_repo_mutation",
		"next_denied_class":         "fully_unsupervised_complex_mutation",
		"rsi":                       "denied",
	})
...
func writeActiveStackGithubRunsReportForTest(t *testing.T, path string, ciOverrides map[string]string) {
	...
	ciRuns := map[string]string{
		...
		"ao-atlas":          "30956775511",
		"ao-forge":          "31017677133",
		"ao-command":        "31027012970",
		"ao2":               "31187008934",
		"ao2-control-plane": "30968880090",
		"ao-covenant":       "31026633959",
	}
	opsRuns := map[string]string{
		"ao-foundry":        "28027968419",
		"ao-atlas":          "30956775480",
		"ao-forge":          "31172789285",
		"ao-command":        "31173234616",
		"ao2":               "31174328265",
		"ao2-control-plane": "31172929879",
		"ao-covenant":       "31173669247",
	}
